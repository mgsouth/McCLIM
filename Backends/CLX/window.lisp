...
(defclass clx-window (clx-mirror)
  ((mirror
    :reader window)
   (sheet
    :reader sheet
...
(defmethod mirror-width ((mirror clx-window))
  (ceiling (bounding-rectangle-width (sheet-mirror-geometry (sheet mirror)))))

(defmethod mirror-height ((mirror clx-window))
  (ceiling (bounding-rectangle-height (sheet-mirror-geometry (sheet mirror)))))

(defmethod clx-drawable ((object clx-window))
  (if (buffering-p object)
      (clx-drawable (pixmap object))
      (window object)))

(defmethod (setf buffering-p) :before (new-value (mirror clx-window))
  (when (and new-value (null (buffering-p mirror)))
    (let* ((width (mirror-width mirror))
           (height (mirror-height mirror))
           (pixmap (ensure-pixmap mirror (pixmap mirror) width height)))
      (setf (pixmap mirror) pixmap)
      (%drawable-copy-area (clx-drawable mirror) 0 0
                           width height
                           (clx-drawable pixmap) 0 0))))

(defun swap-buffers (mirror)
  (let ((pixmap (pixmap mirror)))
    (%drawable-copy-area (clx-drawable pixmap) 0 0
                         (pixmap-width pixmap)
                         (pixmap-height pixmap)
                         (window mirror) 0 0)))

;;; Return a string in which every non-STANDARD-CHAR in STRING has been
;;; replaced with #\_. The result is guaranteed to be an ASCII string.
