...
(defun draw-glyphs/fast (glyph-set glyph-ids start end
                         origin-x origin-y advance-x advance-y
                         medium x y transformation)
  (declare (optimize (speed 3))
           (type index start end)
           (type clx-coordinate origin-x origin-y advance-x advance-y))
...
(defun draw-glyphs/fine (glyph-set glyph-ids start end
                         origin-x origin-y advance-x advance-y
                         medium x y transformation)
  (declare (optimize (speed 3))
           (type index start end)
  ...
  (let* ((port (port medium))
    ...
    (let (origin-x origin-y advance-x advance-y)
      ...
      (if (translation-transformation-p transformation)
          (draw-glyphs/fast glyph-set glyph-ids start end
                            origin-x origin-y advance-x advance-y
                            medium x y transformation)
          (draw-glyphs/fine glyph-set glyph-ids start end
                            origin-x origin-y advance-x advance-y
                            medium x y transformation)))))
