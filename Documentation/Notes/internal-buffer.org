...
"right sticky" cursor positions itself after the new item.
...
introducing concepts like the scan pointer and the insertion pointer.
...
For Silex it may be desireable to provide a simplified implementation that does
not buy into higher level CLIM abstractions. Given all requirements above here
is how the input editing is structured:
...
Standard extended output stream has the associated cursor that may be moved by
the programmer to arbitrary coordinates. We could either create a new buffer for
each such movement, or we may provide our own class for the line. Cluffer uses
~change-class~ to open and close the line, so we can't directly subclass it. We
use a stealth mixin instead.
...
The function ~input-editor-table~ may be specialized to the client (a sheet)
...
https://web.archive.org/web/20221206134053/http://web.mit.edu/~yandros/doc/craft-text-editing/
branches: input-editor, buffer-output
