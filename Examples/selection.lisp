...
(defmethod display ((frame selection-demo) pane)
  ...
  (flet ((show-number (n)
    ...
    (dotimes (v 50)
      ...
      (write-string " " pane)))
  (terpri pane)
  (stream-increment-cursor-position pane 0 30)
  (formatting-item-list (pane :n-columns 2)
    ...
    (formatting-cell (pane)
      (surrounding-output-with-border (pane)
        (with-output-as-gadget (pane)
          (make-pane :text-editor :ncolumns 40 :nlines 4)))))

  (with-translation (pane 720 20)
    (present (make-instance 'image)))
  (with-translation (pane 860 20)
    (present (make-instance 'image :caption "Figure 1: Glider.") 'figure))
  (with-temporary-margins (pane :top '(:absolute 150)
                                :left '(:absolute 720)
                                :right '(:relative 20)
                                :move-cursor nil)
    (setf (stream-cursor-position pane) (values 720 150))
