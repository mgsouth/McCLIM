...
(defun popup-list-box (parent)
  (let* ((frame *application-frame*)
         ...
         (last-click-time nil)
         (last-item-index nil))
    (macrolet ((with-popup (nil &body body)
...
(defmethod handle-event ((pane generic-option-pane) (event pointer-scroll-event))
  (when (list-pane-exclusive-p pane)
    (let ((items (list-pane-items pane)))
      (if-let ((old-position (position (gadget-value pane) items
                                       :test (list-pane-test pane)
                                       :key (list-pane-value-key pane))))
        (let ((new-position (clamp (+ old-position (signum (pointer-event-delta-y event)))
                                   0 (1- (length items)))))
          (unless (= old-position new-position)
            (setf (gadget-value pane :invoke-callback t) (elt items new-position))))
        (unless (emptyp items)
          (setf (gadget-value pane :invoke-callback t) (elt items 0)))))))
