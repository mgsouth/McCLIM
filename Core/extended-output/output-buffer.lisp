...
(defclass edward-sheet-mixin ()
  ((text-cursor :accessor sheet-text-cursor)
   (text-buffer :accessor sheet-text-buffer)
   ;;
   (page-region :initarg :page-region :accessor sheet-page-region)
   (line-direction :initarg :line-direction :accessor sheet-line-direction)
   (page-direction :initarg :page-direction :accessor sheet-page-direction)
   (end-of-line-action :initarg :end-of-line-action :accessor sheet-end-of-line-action)
   (end-of-page-action :initarg :end-of-page-action :accessor sheet-end-of-page-action))
...
(defmethod initialize-instance :after ((self edward-sheet-mixin) &rest args)
  ...
  (let ((buffer nil) ;; not yet
    ...
    (setf (sheet-text-buffer self) buffer)
    (setf (sheet-text-cursor self) cursor)))

(defun sheet-initial-position (sheet)
  (with-bounding-rectangle* (x1 y1 x2 y2) (sheet-page-region sheet)
    (let ((x0 x1) (y0 y1))
      (ecase (sheet-line-direction sheet)
        (:left-to-right (setf x0 x1))
        (:right-to-left (setf x0 x2))
        (:top-to-bottom (setf y0 y1))
        (:bottom-to-top (setf y0 y2)))
      (ecase (sheet-page-direction sheet)
        (:top-to-bottom (setf y0 y1))
        (:bottom-to-top (setf y0 y2))
        (:left-to-right (setf x0 x1))
        (:right-to-left (setf x0 x2)))
...
(defun sheet-final-position (sheet)
  (with-bounding-rectangle* (x1 y1 x2 y2) (sheet-page-region sheet)
    (let ((xn x2) (yn y2))
      (ecase (sheet-line-direction sheet)
...
(defun sheet-text-margin (sheet)
  (ecase (sheet-line-direction sheet)
    ((:left-to-right :right-to-left)
     (bounding-rectangle-width (sheet-page-region sheet)))
    ((:top-to-bottom :bottom-to-top)
     (bounding-rectangle-height (sheet-page-region sheet)))))

(defun sheet-text-offset (sheet cursor)
...
(defun sheet-line-break (sheet cursor text start end)
  (orf start 0)
  (orf end (length text))
  (when (>= start end)
    (return-from sheet-line-break nil))
  (let ((margin (sheet-text-margin sheet))
        (offset (sheet-text-offset sheet cursor))
        (direction (sheet-line-direction sheet))
        (end-of-line (sheet-end-of-line-action sheet)))
...
;;;
(defun sheet-cursor-motion (sheet cursor record &rest args)
  (nest
   (let ((line-direction (sheet-line-direction sheet))
         (page-direction (sheet-page-direction sheet))
         (page-region (sheet-page-region sheet))))
